...
reported rather than passed over in silence. The 'dropped:' map in
'partcad.yaml' beside this file is what words each counter for a reader of
`pc info`. See docs/source/simulation.rst in PartCAD itself.
"""
...
# against: 'primitive_shapes' builds the box, cylinder and sphere a model names
# instead of a mesh, and 'urdf_common' is the pose arithmetic a placement is
# stated in. A sandbox runs a wrapper out of PartCAD's 'wrappers/' directory, so
# both are already on sys.path by the time this is imported.
