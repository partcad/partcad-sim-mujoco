...
MJCF is the one description of a placed arrangement that is both an assembly
type and a scene type: the same element holds a robot and the table it is
bolted to, and nothing in the file says which it is, so the package says so
instead.

Everything here runs without a MuJoCo and without a CAD library. The reader and
the writer are XML and arithmetic; the only thing either needs OCCT for is the
geometry at the ends of it -- the primitives a model names instead of a mesh,
and triangulating what the writer writes -- and both are stubbed, because what
is under test is the mapping between the two models. Running a model is
`simulate_mujoco.py`'s business.

Run it with `pytest`. It needs `partcad` installed, which is where the sandbox
contract these scripts are written against lives: `ocp_serialize` (the shape and
assembly envelopes), `urdf_common` (poses and the packed location) and
`primitive_shapes` (a box, a cylinder or a sphere as geometry) are PartCAD's,
are on `sys.path` for anything PartCAD runs in a sandbox, and are imported here
the same way.
"""
...
# This package's own scripts first, then the sandbox contract out of PartCAD.
# The order is the order a sandbox has them in, which matters for exactly one
# name: `mujoco_common` shipped inside the `partcad` wheel until 0.8.80, and a
# test run against an older PartCAD must still read this repository's copy.
...
def dropped_labels():
    ...
    Read out of `partcad.yaml` rather than repeated here, because that file is
    what PartCAD reads: a wording that drifts from it is a wording nobody sees.
    """
    declaration = yaml.safe_load(open(os.path.join(HERE, "partcad.yaml"), encoding="utf-8"))
    return declaration["import"]["mjcf"].get("dropped") or {}
...
def mjcf_package(tmp_path, monkeypatch):
    """This repository's test package: one MJCF file, as a scene and as an assembly.

    A copy rather than the directory itself, so that a test which rewrites the
    package is not a test that passes once.

    The plugin is declared there as a `local` dependency on `../..`, which is
    this checkout - true where the file sits and false once it has been copied
    somewhere else, so the copy's is rewritten to an absolute path. What a user
    ...
    config["dependencies"]["sim-mujoco"]["path"] = HERE
    with open(root / "partcad.yaml", "w", encoding="utf-8") as f:
