...
Shared by the MJCF reader ('import_mjcf.py') and the MJCF exporter
('export_mjcf.py'), and dependency-free -- no OCP, no mujoco, nothing but the
standard library and PartCAD's own 'urdf_common' -- so it can be imported and
unit-tested on its own. The simulation plugin beside them needs none of it: it
is handed a file and reads it with MuJoCo's own loader.
